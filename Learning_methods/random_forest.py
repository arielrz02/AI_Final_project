...
def rf_cross_val(train: pd.DataFrame, tags: pd.Series, rfparams: dict, fold=5):
    ...
    for i in range(fold):
        ...
        conmat, f1 = rf_single_hyperparams(*train_and_val, rfparams=rfparams)
    ...
    return mean_mat, std_mat, mean_f1, std_f1
...
def evaluate(prediction, tag) -> Tuple[np.ndarray, np.ndarray]:
    con_mat = confusion_matrix(prediction, tag, labels=range(9), normalize="true")
...
def choose_rf_params(df: pd.DataFrame, tags: pd.Series):
    ...
    for n_est, max_feat, max_depth, min_splt, min_leaf in tqdm(product(n_estimators_lst,
        ...
        mean_mat, std_mat, mean_f1, std_f1 = rf_cross_val(df, tags, paramsgrid)
    ...
    return maxconmat, maxconmatstd, conmatparams, maxf1, maxf1std, f1params
...
if __name__ == "__main__":
    ...
    print(f"max of confusion matrix with params {params[2]}")
    print(rf_single_hyperparams(train, test, train_tag, test_tag, params[2]))
    print(f"max of f1 with params {params[5]}")
