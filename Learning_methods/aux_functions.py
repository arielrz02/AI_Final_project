...
def loading_data(x_train, y_train, x_validation, y_validation, batch_size):
    ...
    validation_data = testData(torch.FloatTensor(x_validation), torch.FloatTensor(y_validation))
...
def calculate_weighted_in_train(y_train, options):
    weighted = np.zeros(options)
    for i in range(options):
        weighted = list(y_train).count(i)

    return torch.tensor(weighted)  # check
